 ...
 */	 
	public class TweenLite extends TweenCore {
		...
		/** Easing method to use which determines how the values animate over time. Examples are Elastic.easeOut and Strong.easeIn. Many are found in the fl.motion.easing package or com.greensock.easing. **/
		...
		protected function init():void {
			var p:String, i:int, plugin:*, prioritize:Boolean;
			propTweenLookup = {};
			if (this.target is TweenCore && "timeScale" in this.vars) { //in case we're tweening the timeScale of another tween/timeline.
			...
			for (p in this.vars) {
				if (p in _reservedProps) { 
					...
					if (plugin.onInitTween(this.target, this.vars[p], this) == false) { //if the plugin init fails, do a regular tween
						_firstPropTween = insertPropTween(this.target, p, this.target[p], this.vars[p], p, false, _firstPropTween); 
					} else {
